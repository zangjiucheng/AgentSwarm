...
function start_audio_in (){
	if [[ ${KASM_SVC_AUDIO_INPUT:-1} == 1 ]]; then
		...
		$STARTUPDIR/audio_input/kasm_audio_input_server --auth-token "kasm_user:$VNC_PW" --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
...
function start_upload (){
	if [[ ${KASM_SVC_UPLOADS:-1} == 1 ]]; then
		...
		$STARTUPDIR/upload_server/kasm_upload_server --auth-token "kasm_user:$VNC_PW" --port 4902 --upload_dir ${HOME}/Uploads &
...
function start_gamepad (){
	if [[ ${KASM_SVC_GAMEPAD:-1} == 1 ]]; then
		...
		$STARTUPDIR/gamepad/kasm_gamepad_server --auth-token "kasm_user:$VNC_PW" --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
...
function start_webcam (){
	if [[ ${KASM_SVC_WEBCAM:-1} == 1 ]] && [[ -e /dev/video0 ]]; then
		log 'Starting webcam server'
                if [[ $DEBUG == true ]]; then
			$STARTUPDIR/webcam/kasm_webcam_server --debug --port 4905 --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
		else
			$STARTUPDIR/webcam/kasm_webcam_server --port 4905 --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
		fi
...
function start_nginx (){
	local nginx_process="/usr/sbin/nginx"

	if [ ! -x "$nginx_process" ]; then
		log "${nginx_process} not found or not executable, skipping nginx startup" "WARNING"
		return
	fi
	...
	if ! sudo "$nginx_process" -t; then
		log "nginx configuration test failed, skipping nginx startup" "ERROR"
		return
	fi

	sudo "$nginx_process" -g "daemon off;" &
	KASM_PROCS['nginx']=$!
	log "Started nginx."
}

function start_monitor (){
	local monitor_process="/opt/monitor/bin/monitor"
	local monitor_log="/home/kasm-user/monitor.log"

	if [ ! -x "$monitor_process" ]; then
		log "${monitor_process} not found or not executable, skipping monitor startup" "WARNING"
		return
	fi
	...
	KASM_PROCS['monitor']=$!
	log "Started monitor process. Logging to ${monitor_log}."
}
...
# Some startup helpers assume these user directories always exist.
mkdir -p "$HOME/Desktop" "$HOME/Downloads" "$HOME/Uploads"
		...
		if ! kill -0 "${KASM_PROCS[$process]}" ; then
			...
			case $process in
				...
				custom_script)
					...
					;;
				monitor)
					log "Monitor process exited, restarting" "WARNING"
					start_monitor
					;;
				nginx)
					log "nginx exited, restarting" "WARNING"
					start_nginx
					;;
				*)
